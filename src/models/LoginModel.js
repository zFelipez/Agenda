...
class Login {
    ...
    async login(){
        ...
        if(!bcryptjs.compareSync(this.body.password, this.user.password)){
            this.errors.push('Usuario ou senha inavalidos')
            this.user = null
            return;
        }
    }
    ...
    async register() {
        this.valida();
        if (this.errors.length > 0) return; //checa primeiro se tem algum erro 

        await this.userExists()

        if (this.errors.length > 0) return;// checa se o usuario existe 

        const salt = bcryptjs.genSaltSync();
       
        this.body.password = bcryptjs.hashSync(this.body.password, salt); // criptopgrafa a senha              
